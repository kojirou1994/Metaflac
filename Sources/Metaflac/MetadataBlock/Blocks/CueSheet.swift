...
extension FlacMetadataBlock {
  ...
  public struct CueSheet: FlacMetadataBlockProtocol, Equatable {
    ...
    public init(mediaCatalogNumber: String, numberOfLeadinSamples: UInt64,
      ...
      precondition(mediaCatalogNumber.utf8.count <= 128)
      assert(mediaCatalogNumber.utf8.allSatisfy{ (0x20...0x7e).contains($0) })
      self.mediaCatalogNumber = .init(mediaCatalogNumber.utf8)
    ...
    public init<D>(_ data: D) throws where D : DataProtocol {
      ...
      do {
        let catalogNumber = try reader.read(128)
        let endIndex = catalogNumber.firstIndex(of: 0) ?? catalogNumber.endIndex
        mediaCatalogNumber = .init(catalogNumber[..<endIndex])
      }
      ...
      try reader.skipReservedBytes(258)
      ...
      tracks = try (1...trackCount).map { _ in
        ...
        // Reserved. All bits must be set to zero.
        try reader.skipReservedBytes(13)
        ...
        let indexes = try (1...numberOfTrackIndexPoints).map { _ -> Track.Index in
          ...
          // Reserved. All bits must be set to zero.
          try reader.skipReservedBytes(3)
    ...
    public var encodedBytes: Data {
      ...
      if case let zeroCount = 128 - mediaCatalogNumber.count, zeroCount > 0 {
        result += [UInt8](repeating: 0, count: zeroCount)
      }
